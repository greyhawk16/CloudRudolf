...
resource "aws_iam_policy" "cr-fd-web-ec2-role-policy" {
  name = "cr-fd-web-ec2-role-policy"
  description = "cr-fd-web-ec2-role-policy"
  policy = <<POLICY
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_policy_attachment" "cr-fd-web-ec2-role-policy-attachment" {
  name = "cr-fd-web-ec2-role-policy-attachment"
  roles = [
...
resource "aws_iam_instance_profile" "cr-fd-web-ec2-instance-profile" {
  name = "cr-fd-web-ec2-instance-profile"
...
resource "aws_security_group" "cr-fd-web-ec2-security-group" {
  name        = "cr-fd-web-ec2-security"
  description = "Clou} Security Group for file download web EC2 Instance over SSH, http"
...
resource "aws_instance" "cr-fd-web-ec2" {
  ...
  user_data = <<-UD
    ...
    echo -e "${aws_iam_access_key.cr-fd-ac-sonic.id}\n${aws_iam_access_key.cr-fd-ac-sonic.secret}\nus-east-1\njson" |sudo -u ubuntu aws configure --profile sonic
    sudo apt install openjdk-21-jdk -y
    cd /home/ubuntu/code/fd_spring
    sudo -u ubuntu gradle build
    sudo -u ubuntu java -jar build/libs/fd_spring-0.0.1-SNAPSHOT.jar &
  UD
...
resource "aws_security_group" "cr-fd-flag-ec2-security-group" {
  name        = "cr-fd-flag-ec2"
  description = "Cloud Security Group for file download flag EC2 Instance"
  vpc_id      = "${aws_vpc.cr-vpc.id}"
}
